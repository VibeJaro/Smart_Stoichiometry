...
function computeTheoreticalYield(limitingReagent, product) {
  ...
  const targetMolarMass = product?.molarMass || limitingReagent.molarMass;
  const grams = limitingReagent.moles * targetMolarMass;
  return {
    massGrams: grams,
    description: product
      ? `Theoretical yield assuming 1:1 stoichiometry for ${product.canonicalName} based on ${limitingReagent.canonicalName}.`
      : `Theoretical yield assuming 1:1 stoichiometry based on ${limitingReagent.canonicalName}.`,
  };
}
