...
function isCasNumber(identifier) {
  return typeof identifier === 'string' && /^\d{2,7}-\d{2}-\d$/.test(identifier.trim());
}

function buildPubChemUrls(identifier) {
  const encoded = encodeURIComponent(identifier);
  const base = 'https://pubchem.ncbi.nlm.nih.gov/rest/pug/compound';
  if (isCasNumber(identifier)) {
    return {
      propertyUrl: `${base}/xref/RN/${encoded}/property/MolecularWeight,IUPACName,IsomericSMILES/JSON`,
      recordUrl: `${base}/xref/RN/${encoded}/JSON`,
      casNumber: identifier,
    };
  }
  return {
    propertyUrl: `${base}/name/${encoded}/property/MolecularWeight,IUPACName,IsomericSMILES/JSON`,
    recordUrl: `${base}/name/${encoded}/JSON`,
    casNumber: null,
  };
}

async function fetchPubChemProperties(identifier, fetchFn) {
  // PUG REST: https://pubchem.ncbi.nlm.nih.gov/rest/pug/compound/{name|xref}/.../property/{PropertyList}/JSON
  // Density/solubility/boiling/melting points are pulled from the full JSON record (/JSON).
  const { propertyUrl, recordUrl, casNumber: preResolvedCas } = buildPubChemUrls(identifier);
  ...
  const [casNumber, physicalProps] = await Promise.all([
    preResolvedCas ?? fetchCasNumber(identifier, fetchFn),
    fetchPhysicalProperties(identifier, fetchFn, recordUrl),
  ]);
  return {
    ...
    boilingPointC: physicalProps.boilingPointC,
    meltingPointC: physicalProps.meltingPointC,
...
async function fetchPhysicalProperties(identifier, fetchFn, recordUrl) {
  const url =
    recordUrl || `https://pubchem.ncbi.nlm.nih.gov/rest/pug/compound/name/${encodeURIComponent(identifier)}/JSON`;
  try {
    const response = await fetchFn(url, { headers: { accept: 'application/json' } });
